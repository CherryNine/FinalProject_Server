...
let UsersService = class UsersService {
    ...
    async updateStatus(ids, status) {
        ...
        const statusValue = status === 'blocked' ? client_1.Status.BLOCKED : client_1.Status.UNBLOCKED;
        for (const id of idArray) {
            updatePromises.push(this.usersRepo.updateStatus(id, statusValue));
        }
        return Promise.all(updatePromises);
    }
    async changeRole(userIds, newRole) {
        const userPromises = userIds.map((userId) => this.usersRepo.findById({ id: userId }).then((targetUser) => {
            if (!targetUser) {
        ...
        const updatePromises = users.map((user) => this.usersRepo.updateRole(user.id, newRole));
        return Promise.all(updatePromises);
    }
};
